...
function mobile_discuzcode($param) {
	...
	if(!$bbcodeoff) {
		...
		if(strpos($msglower, '[/img]') !== FALSE) {
            ...
            $callbacks = $allowimgcode ? array(
                function($matches) use ($lazyload, $pid) {
                    return mobile_parseimg(0, 0, $matches[1], intval($lazyload), intval($pid),
                        'onmouseover="img_onmouseoverfunc(this)"' . ($lazyload ? 'lazyloadthumb="1"' : 'onload="thumbImg(this)"'));
                },
                function($matches) use ($lazyload, $pid) {
                    return mobile_parseimg($matches[1], $matches[2], $matches[3], intval($lazyload), intval($pid));
                }
            ) : ($allowbbcode ? array(
                function($matches) {
