	...
	function discuzcode($param) {
		...
		if($param['caller'] == 'discuzcode') {
			...
			if(!$smileyoff && $allowsmilies && strpos($_G['discuzcodemessage'], '{:soso_') !== false) {
				$_G['discuzcodemessage'] = preg_replace_callback("/\{\:soso_((e\d+)|(_\d+_\d))\:\}/", function($matches) use ($_G, $pid) {
                    return $this->_soso_smiles($matches[1], $_G['setting']['maxsmilies'], $pid);
                }, $_G['discuzcodemessage'], $_G['setting']['maxsmilies']);
			}
		} else {
...
class plugin_soso_smilies_home extends plugin_soso_smilies {
	...
	function spacecp_profile_sightml() {
		...
		if($_GET['ac'] == 'profile' && submitcheck('profilesubmitbtn') && !empty($_POST['sightml'])) {
			$_POST['sightml'] = preg_replace_callback("/\{\:soso_((e\d+)|(_\d+_\d))\:\}/", function($matches) {
                return $this->_soso_smiles($matches[1], -1, 0, 1);
            }, $_POST['sightml'], $_G['setting']['maxsmilies']);
		}
	}
		...
		if(!empty($GLOBALS['msglist'])) {
			foreach($GLOBALS['msglist'] as $day => $result) {
				foreach($result as $key => $value) {
					$GLOBALS['msglist'][$day][$key]['message'] = preg_replace_callback("/\{\:soso_((e\d+)|(_\d+_\d))\:\}/", function($matches) {
                        return $this->_soso_smiles($matches[1], -1, 0, 0);
                    }, $GLOBALS['msglist'][$day][$key]['message'], $_G['setting']['maxsmilies']);
				}
			}
		} elseif($_GET['op'] == 'showchatmsg' && $GLOBALS['list']) {
			foreach($GLOBALS['list'] as $key => $value) {
				$GLOBALS['list'][$key]['message'] = preg_replace_callback("/\{\:soso_((e\d+)|(_\d+_\d))\:\}/", function($matches) {
                    return $this->_soso_smiles($matches[1], -1, 0, 0);
                }, $GLOBALS['list'][$key]['message'], $_G['setting']['maxsmilies']);
			}
		}
	}
	...
	function space_pm_output() {
		...
		if(!empty($GLOBALS['list'])) {
			foreach($GLOBALS['list'] as $key => $value) {
				if(!empty($_GET['subop'])) {
					$GLOBALS['list'][$key] = preg_replace_callback("/\{\:soso_((e\d+)|(_\d+_\d))\:\}/", function($matches) {
                        return $this->_soso_smiles($matches[1], -1, 0, 0);
                    }, $GLOBALS['list'][$key], $_G['setting']['maxsmilies']);
				} else {
	...
	function follow_soso_output() {
		...
		if(!empty($GLOBALS['list']['content'])) {
			foreach($GLOBALS['list']['content'] as $key => $value) {
				$GLOBALS['list']['content'][$key]['content'] = preg_replace_callback("/\{\:soso_((e\d+)|(_\d+_\d))\:\}/", function($matches) {
                    return $this->_soso_smiles($matches[1], -1, 0, 0);
                }, $GLOBALS['list']['content'][$key]['content'], $_G['setting']['maxsmilies']);
			}
		}
	}
