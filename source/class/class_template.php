...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		$template = preg_replace_callback("/\{hook\/(\w+?)(\s+(.+?))?\}/i", function($matches) {
            if(!empty($matches[3])) return $this->hooktags($matches[1], $matches[3]);
        }, $template);
		$template = preg_replace_callback("/$var_regexp/s", function($matches) {
            return template::addquote('<?=' . $matches[1] . '?>');
        }, $template);
		$template = preg_replace_callback("/\<\?\=\<\?\=$var_regexp\?\>\?\>/s", function($matches) {
	...
	function loadcsstemplate() {
		...
		$content = preg_replace_callback("/\[(.+?)\](.*?)\[end\]/is", function($matches) {
