...
function html2bbcode($text) {
	...
	$pregfind = array(
        "/<table([^>]*(width|background|background-color|bgcolor)[^>]*)>/siU",
        "/<td(.+)>/siU",
        '/<h([0-9]+)[^>]*>/siU',
    );
    $pregreplace = array(
    ...
    $pregreplace = array(
        function($matches) { return smileycode($matches[1]); },
        function($matches) { return imgtag($matches[1]); }
    );
...
function parsestyle($tagoptions, &$prependtags, &$appendtags) {
	...
	$style = preg_replace_callback(
        ...
        function($matches) {
            return sprintf("color: #%02X%02X%02X$4", $matches[1], $matches[2], $matches[3]);
        },
		$style
	);
